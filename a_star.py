...
class A_star:
    ...
    def run(self, grid):
        ...
        open_cell[start] = (
            distance  # We pass in as (distance, cell) so that the heap is sorted by distance
        )
        ...
        while len(open_cell):
            ...
            for n in neighbors:
                ...
                new_distance = cell.distance_from_start + distance_between_cells(
                    cell, neighbor_cell
                )  # g(x)
                ...
                if neighbor_cell.distance_from_end == None:
                    neighbor_cell.distance_from_end = distance_between_cells(
                        neighbor_cell, end
                    )  # h(x)

                evaluation = (
                    neighbor_cell.distance_from_end + neighbor_cell.distance_from_start
                )  # f(x) = g(x) + h(x)
